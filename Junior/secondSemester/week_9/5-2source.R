Sys.setlocale(category = "LC_ALL",locale = "Korean_Korea.UTF-8")

# dplyr 로드
library(dplyr)

#readxl 로드 및 엑셀파일 불러오기
library(readxl)
exdata1 <- read_excel("C:/tukorea_university_ojt/Junior/secondSemester/week_9/Sample1.xlsx")
exdata1


# 선택한 변수 추출하기
exdata1 %>% select(ID)


# 선택한 변수 여러 개 추출하기
exdata1 %>% select(ID, AREA, Y21_CNT)


# 선택한 변수 제외하고 추출하기
exdata1 %>% select(-AREA)

# 선택한 변수 제외하고 추출하기2
exdata1 %>% select(-AREA, - Y21_CNT)


# 조건에 만족하는 데이터만 추출하기
exdata1 %>% filter(AREA == '서울')

# 조건에 만족하는 데이터만 추출하기2
exdata1 %>% filter(AREA == '서울' & Y21_CNT >= 10)


# 오름차순 정렬하기
exdata1 %>% arrange(AGE)


# 내림차순 정렬하기
exdata1 %>% arrange(desc(Y21_AMT))


# 중첩 정렬하기
exdata1 %>% arrange(AGE, desc(Y21_AMT))


# 변수 값 합산하기
exdata1 %>% summarise(TOT_Y21_AMT = sum(Y21_AMT))


# 변수 값을 그룹별로 합산하기
exdata1 %>% group_by(AREA) %>% summarise(SUM_Y21_AMT = sum(Y21_AMT))


# 엑셀 파일 불러오기
library(readxl)
m_history <- read_excel("C:/tukorea_university_ojt/Junior/secondSemester/week_9/Sample2_m_history.xlsx")
f_history <- read_excel("C:/tukorea_university_ojt/Junior/secondSemester/week_9/Sample3_f_history.xlsx")
View(m_history)
View(f_history)


# 테이블을 세로로 결합하기
exdata_bindjoin <- bind_rows(m_history, f_history)
View(exdata_bindjoin)


# 엑셀 파일 불러오기
library(readxl)
jeju_y21_history <- read_excel("C:/tukorea_university_ojt/Junior/secondSemester/week_9/Sample4_y21_history.xlsx")
jeju_y20_history <- read_excel("C:/tukorea_university_ojt/Junior/secondSemester/week_9/Sample5_y20_history.xlsx")
View(jeju_y21_history)
View(jeju_y20_history)


# 첫 번째 테이블 기준으로 가로 결합하기
bind_col <- left_join(jeju_y21_history, jeju_y20_history, by = "ID")
View(bind_col)


# 키 변수가 동일할 때만 가로 결합하기
bind_col_inner <- inner_join(jeju_y21_history, jeju_y20_history, by = "ID")
View(bind_col_inner)


# 키 변수를 기준으로 모두 가로 결합하기
bind_col_full <- full_join(jeju_y21_history, jeju_y20_history, by = "ID")
View(bind_col_full)